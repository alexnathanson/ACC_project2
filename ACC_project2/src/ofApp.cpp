...
void ofApp::update() {
	...
	if (bNewFrame) {
		...
		contourFinder.findContours(grayDiff, minContour, (320 * 240) / 3, contAmt, false);
...
void ofApp::draw() {
	...
	switch (effect) {
	case 1: drawSceneOneLocal(allBlobs);
			drawSceneOneRemote(allRem);
			break;
	case 2: ofSetColor(urColor);
			drawPoints(allBlobs);
			ofSetColor(remColor);
			drawPoints(allRem);
			drawBodyContour(allBlobs, urColor);
...
void ofApp::storeMessage(string inMess) {
	...
	if (inMess != "") {
		...
		if (inMess.substr(0, idLen) == myIP[0]) {
			...
			if (remIP[0].size() != 0) {
				int ipStart = inMess.find(remIP[0]);
				inMess = inMess.erase(ipStart, remSize);
			}
...
void ofApp::drawSceneOneRemote(vector<vector <ofPoint> > points) {
	vector<ofPolyline> polylines = getBodyContour(points);
	ofPushMatrix();
		ofSetColor(255);
		...
		for (auto & line : polylines) {
			ofRectangle r = line.getBoundingBox();
			float w = r.getWidth();
			float h = r.getHeight(); 
			ofPushMatrix();
			ofTranslate(ofGetWidth() / 2, ofGetHeight() / 2);
